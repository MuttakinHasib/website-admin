angular.module('App').controller('AddTopicController', function ($rootScope, $scope, $http, $mdToast, $mdDialog, $route, $timeout, request) {

	// define variable
	var self 		= $scope, root = $rootScope;
	var is_new 		= ( root.getCurTopicId() == null );
	var original 	= null;
	...
	root.pagetitle 			= (is_new) ? 'Add Topic' : 'Edit Topic';
	...
	if (is_new) {
		...
		original = { name: null, icon: null, draft: 1, color: "#494949", featured: 0, priority: null, created_at: now, last_update: now };
		self.topic = angular.copy(original);
	} else {
		self.icon.valid = true;
		request.getOneTopic(root.getCurTopicId()).then(function(resp){
			original = resp.data;
			self.topic = angular.copy(original);
		});
	}
	...
	self.submit = function(c) {
		...
		if(is_new){ // create
			...
			request.insertOneTopic(c).then(function(resp){
			...
			request.updateOneTopic(c.id, c).then(function(resp){
				self.resp_submit = resp;
				if(resp.status == 'success'){
					self.done_arr[0] = true;
	...
	self.isReadySubmit = function () {
		self.is_clean = angular.equals(original, self.topic);
		if (is_new) {
	...
	self.cancel = function () { window.location.href = '#topic'; };
